...
public class TableDataRepositoryImpl implements TableDataRepository {
    ...
    @Autowired
    public TableDataRepositoryImpl(String schema, NamedParameterJdbcOperations sourceJdbcTemplate, MetadataRepository metadataRepository) {
        this.schema = schema;
    ...
    public void apply(Map<RowId, RowAction> rowActionResult, TableData mirrorTable, TableData sourceTable) {
        ...
        if (primaryKeyMetadatas.size() != 1) {
            String columnNames = Joiner.on(", ").join(primaryKeyMetadatas.stream().map(o -> o.getColumn().getName()).collect(Collectors.toList()));
    ...
    public TableData get(TableNameIdentifier tableNameIdentifier, Table<RowId, String, Object> keysForOtherPartTable) {
        Map<String, String> columnToFieldMap = keysForOtherPartTable.columnKeySet().stream().collect(Collectors.toMap(o -> o, o -> o));
    ...
    public void persist(TableData sourceTable, Map<RowId, RowAction> rowActionResult, TableData destination) {
        ...
        String sql = String.format("insert into %s(%s) values(%s)", metadata.getTableName().getFullQualifiedName(),
    ...
    private List<String> getInsertStatements(TableData tableData, Map<RowId, RowAction> rowActionResult, TableData destination,
            SqlParameterFormatter formatter) {
        TableMetadata metadata = destination.getMetadata();
        List<ColumnMetaData> columns = metadata.getColumns();

        String sql = String.format("insert into %s(%s) values(%s)", metadata.getTableName().getFullQualifiedName(),
                Joiner.on(", ").join(columns.stream().map(o -> o.getName()).collect(Collectors.toList())), "%s");
    ...
    private String buildSelectOnDownstream(TableNameIdentifier tableIdentifier,
        ...
        for (List<DownstreamReferenceColumnMetaData> downstreamColumnConstriant : downstreamColumnConstriants) {
            ...
            for (DownstreamReferenceColumnMetaData downstreamColumn : downstreamColumnConstriant) {
                ...
                sqlBuilder.append(String.format("%s is null or %s = :%s", downstreamColumn.getColumnName(), downstreamColumn.getColumnName(),
                        downstreamColumn.getForeignColumnName()));
                sqlBuilder.append(")");
            }
            sqlBuilder.append(")");
        }
    ...
    private String buildSelectOnUpstream(TableNameIdentifier tableIdentifier, List<List<UpstreamReferenceColumnMetaData>> upstreamColumnConstriants) {
        StringBuilder sqlBuilder = getSelectAllBuilder(tableIdentifier);
        sqlBuilder.append("\r\n");
        ...
        for (List<UpstreamReferenceColumnMetaData> upstreamColumnConstriant : upstreamColumnConstriants) {
            ...
            for (UpstreamReferenceColumnMetaData upstreamColumn : upstreamColumnConstriant) {
                sqlBuilder.append(" or (");
                sqlBuilder.append(String.format("%s is null or %s = :%s", upstreamColumn.getColumnName(), upstreamColumn.getColumnName(),
                        upstreamColumn.getForeignColumnName()));
                sqlBuilder.append(")");
            }
            sqlBuilder.append(")");
        }
    ...
    private List<Map<String, Object>> readDataDistinceOnPrimaryKey(TableData tableData, String sql, TableMetadata metaData) {
        ...
        for (Map<String, Object> row : tableData) {
            ...
            for (final Map<String, Object> subResultRow : subResult) {
                List<Object> primaryKey = metaData.getPrimaryKeys().stream().map(o -> subResultRow.get(o.getColumn().getName()))
    ...
    private String buildSelectOnPrimaryKey(TableNameIdentifier tableIdentifier, List<PrimaryKeyMetadata> primaryKeys) {
        ...
        for (PrimaryKeyMetadata primaryKey : primaryKeys) {
            sqlBuilder.append(" or (");
            sqlBuilder.append(String.format("%s is null or %s = :%s", primaryKey.getColumn().getName(), primaryKey.getColumn().getName(),
