...
@ComponentScan(basePackageClasses = { Application.class }, excludeFilters = {
        @ComponentScan.Filter(type = FilterType.REGEX, pattern = { "dk.emstar.data.datadub.configuration.*" }) })
public class SpringConfig {
    ...
    @Bean
    public TableDataRepository sourceTableDataRepository(@Value("${source.schema}") String schema,
            @Qualifier("sourceJdbcTemplate") NamedParameterJdbcOperations jdbcOperations,
    ...
    @Bean
    public TableDataRepository destinationTableDataRepository(@Value("${destination.schema}") String schema,
            @Qualifier("destinationJdbcTemplate") NamedParameterJdbcOperations jdbcOperations,
