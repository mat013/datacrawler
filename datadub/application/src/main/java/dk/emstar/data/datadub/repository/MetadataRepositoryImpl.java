...
public class MetadataRepositoryImpl implements MetadataRepository {
    ...
    public MetadataRepositoryImpl(@Qualifier("sourceDataSource") DataSource dataSource) {
        this(dataSource, new ColumnMetadataRowMapper(), new UpstreamReferenceColumnMetadataRowMapper(),
                new DownstreamReferenceColumnMetadataRowMapper());
    }

    // TODO use the correct constructor
    public MetadataRepositoryImpl(@Qualifier("sourceDataSource") DataSource dataSource, ColumnMetadataRowMapper columnMetadataRowMapper,
    ...
    public TableMetadata getMetaData(TableNameIdentifier tableNameIdentifier) {
        try (Connection connection = dataSource.getConnection()) {
            ...
            ResultSet tableResultSet = metaData.getTables(tableNameIdentifier.getCatalog(), tableNameIdentifier.getSchema(),
                    tableNameIdentifier.getTableName(), new String[] { "TABLE" });
            while (tableResultSet.next()) {
                ...
                Map<String, List<DownstreamReferenceColumnMetaData>> downstreamReferenceColumns = getDownstreamReferences(metaData,
                        tableNameIdenfier);
                ...
                tableMetaDataList
                        .add(new TableMetadata(tableNameIdenfier, columns, primaryKeys, upstreamReferenceColumns, downstreamReferenceColumns));
            }
    ...
    private List<PrimaryKeyMetadata> getPrimaryKeys(DatabaseMetaData metaData, TableNameIdentifier tableNameIdenfier, List<ColumnMetaData> columns)
            throws SQLException {

        Map<String, ColumnMetaData> map = columns.stream().collect(Collectors.toMap(ColumnMetaData::getName, a -> a));

        PrimaryKeyMetadataRowMapper mapper = new PrimaryKeyMetadataRowMapper(map);

        List<PrimaryKeyMetadata> result = Lists.newArrayList();
        try (ResultSet resultSet = metaData.getPrimaryKeys(tableNameIdenfier.getCatalog(), tableNameIdenfier.getSchema(),
                tableNameIdenfier.getTableName())) {
            int count = 0;
    ...
    private Map<String, List<DownstreamReferenceColumnMetaData>> getDownstreamReferences(DatabaseMetaData metaData,
            TableNameIdentifier tableNameIdenfier) throws SQLException {
        Map<String, List<DownstreamReferenceColumnMetaData>> result = Maps.newHashMap();
        try (ResultSet resultSet = metaData.getExportedKeys(tableNameIdenfier.getCatalog(), tableNameIdenfier.getSchema(),
                tableNameIdenfier.getTableName())) {
            int count = 0;
            while (resultSet.next()) {
    ...
    private Map<String, List<UpstreamReferenceColumnMetaData>> getUpstreamReferences(DatabaseMetaData metaData, TableNameIdentifier tableNameIdenfier)
            throws SQLException {
        Map<String, List<UpstreamReferenceColumnMetaData>> result = Maps.newHashMap();
        try (ResultSet resultSet = metaData.getImportedKeys(tableNameIdenfier.getCatalog(), tableNameIdenfier.getSchema(),
                tableNameIdenfier.getTableName())) {
            int count = 0;
            while (resultSet.next()) {
    ...
    private List<ColumnMetaData> getColumns(DatabaseMetaData metaData, TableNameIdentifier tableNameIdenfier) throws SQLException {
        List<ColumnMetaData> columns = Lists.newArrayList();
        try (ResultSet columnResultSet = metaData.getColumns(tableNameIdenfier.getCatalog(), tableNameIdenfier.getSchema(),
                tableNameIdenfier.getTableName(), null)) {
            int count = 0;
